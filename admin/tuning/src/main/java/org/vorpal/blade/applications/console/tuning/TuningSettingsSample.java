...
/// `./config/custom/vorpal/blade-tuning.json` on first deployment when no
...
///
/// Neither profile caps Metaspace. install.sh leaves it unbounded because a
/// MaxMetaspaceSize cap has OOMed a full BLADE deploy partway through; the
/// initial size is kept so the first metaspace GC does not fire during startup.
