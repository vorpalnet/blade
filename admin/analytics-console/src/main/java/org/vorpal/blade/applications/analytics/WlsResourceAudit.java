...
public class WlsResourceAudit {
	...
	/// Looks across every JMS module's resource list for a destination bound to
	/// `expectedJndi`, searching each of the given JMSResource collection
	/// attributes (e.g. "ConnectionFactories", or the three queue flavors
	/// "UniformDistributedQueues" / "DistributedQueues" / "Queues").
	private static Finding auditJmsResource(MBeanServer mbs, ObjectName domain,
			String label, String key, String expectedJndi, String... resourceAttrs) {
		try {
			ObjectName[] mods = (ObjectName[]) mbs.getAttribute(domain, "JMSSystemResources");
					...
					for (ObjectName item : items) {
						...
						if (expectedJndi.equals(jndi)) {
							...
							return new Finding(key, label, true,
									"found '" + itemName + "' (" + resourceAttr + ") in module '"
											+ moduleName + "' bound to " + expectedJndi);
						}
					}
				}
			}
