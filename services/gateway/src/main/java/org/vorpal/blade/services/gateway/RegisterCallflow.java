...
public class RegisterCallflow extends TrunkRegistrar {
	...
	public void start(InetSocketAddress outboundInterface) throws ServletException, IOException {
		...
		// Never expire: this session hosts the recurring re-REGISTER timer
		// (framework convention: setExpires(0) = never). Otherwise OCCAS reaps it
		// on the default SIP app-session timeout, cancelling the timer with it.
		sas.setExpires(0);
	...
	private void onResponse(SipServletResponse response) throws ServletException, IOException {
		...
		if (status == 401 || status == 407) {
			...
			retry.setRequestURI(getSipFactory().createURI(
					"sip:" + gateway.getRegistrarDomain() + ";transport=" + gateway.getTransport()));
			bindOutbound(retry);
			retry.addAuthHeader(response, style.getAuthName(),
					CredentialEncryption.plaintext(style.getPassword()));
		...
		if (successful(response)) {
			...
			// SIP request-processing context), not in servletCreated: a timer
			// created during servlet init does not fire on this container (proven
			// 2026-07-22), whereas one armed in a request callback does — same
			// context as the working AgentCallflow transfer poller. Resolve the
			// session by the stashed id, NOT response.getApplicationSession(): a
			// REGISTER is a non-dialog transaction and OCCAS returns a null app
			// session on its response, which silently skipped the arm before.
			if (timerId == null) {
				SipApplicationSession sas = getSipUtil().getApplicationSessionById(sasId);
				if (sas != null) {
					long periodMs = Math.max(30, requested - REFRESH_MARGIN_SECONDS) * 1000L;
					timerId = startTimer(sas, periodMs, periodMs, false, false, this::onTimer);
					sipLogger.info("gateway " + name() + ": refresh timer armed, period "
							+ (periodMs / 1000) + "s");
				} else {
					sipLogger.severe("gateway " + name()
							+ ": cannot arm refresh timer — session " + sasId + " not found");
				}
			}
