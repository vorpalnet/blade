...
CREATE TABLE applications(
   ...
   -- a restart is a new instance, deliberately. The consumer resolves on these
   -- four columns; without the constraint, two cluster members racing the same
   -- application.started would each insert their own row.
...
CREATE TABLE events(
   ...
   -- The CloudEvent id, so a redelivery does not become a second row. A durable
   -- topic subscription redelivers as a matter of course; `events` has no other
   -- natural key to collide on. Nullable — NULLs do not collide in a UNIQUE index.
   event_uid      CHAR(36 CHAR),
