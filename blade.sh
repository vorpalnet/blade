...
# Ensure the AdminServer launches (via Node Manager) with enough heap/metaspace.
# Node Manager reads a server's own launch args from its startup.properties
# (Arguments=) — that's the NM-aware place to set them. The OCCAS dev default
# (-Xmx512m -XX:MaxMetaspaceSize=256m) OOMs on Metaspace when the admin EAR (10
# WARs) deploys. Override with admin.memory in occas.conf. (Managed/engine
# servers instead take ServerStart Arguments, which the AdminServer hands to NM.)
ensure_admin_memory() {
    local domhome="$1" mem sp
    mem="$(read_prop "$OCCAS_CONF" admin.memory)"
    mem="${mem:--Xms512m -Xmx1024m -XX:MaxMetaspaceSize=512m}"
    sp="${domhome}/servers/AdminServer/data/nodemanager"
    mkdir -p "$sp"
    set_conf_prop "${sp}/startup.properties" Arguments "$mem"
    log "  ${C_DIM}AdminServer launch memory (Node Manager startup.properties): ${mem}${C_RESET}"
}
...
nm_admin() {
    ...
    [ "$action" = "start" ] && { register_domain_with_nm "$dom" "$domhome" || true; ensure_admin_memory "$domhome"; }
