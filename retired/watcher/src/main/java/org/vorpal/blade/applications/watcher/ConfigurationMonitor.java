...
/**
 * File-system watcher that auto-publishes BLADE config files via JMX
 * whenever they change on disk. Direct copy of the working version of the
 * old console app's monitor — see this module's README for the migration
 * rationale.
 */
public class ConfigurationMonitor extends Thread {
	...
	public void initialize(Path dir, boolean recursive) throws IOException {
		System.out.println("BLADE watcher, ConfigurationMonitor dir=" + dir + ", recursive=" + recursive);
	...
	public void updateManagedMBeans(Path path, String json) {
		...
		if (domain || cluster || server) {
			...
			try {
				...
				System.out.println(
						"Please verify that watcher is deployed to the AdminServer, not a managed engine tier node.");
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				if (ctx != null) {
