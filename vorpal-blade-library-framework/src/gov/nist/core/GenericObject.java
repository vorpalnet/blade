...
/**
 * The base class from which all the other classes in the sipheader, sdpfields
 * and sipmessage packages are extended. Provides a few utility funcitons such
 * as indentation and pretty printing that all other classes benifit from.
 *
 * @version 1.2
 *
...
public abstract class GenericObject implements Serializable, Cloneable {
	...
	static final String[] immutableClassNames = { "String", "Character", "Boolean", "Byte", "Short", "Integer", "Long",
			"Float", "Double" };
	...
	/**
	 * Set the pattern matcher. To match on the field of a sip message, set the
	 * match expression in the match template and invoke the match function. This
	 * useful because SIP headers and parameters may appear in different orders and
	 * are not necessarily in canonical form. This makes it hard to write a pattern
	 * matcher that relies on regular expressions alone. Thus we rely on the
	 * following strategy i.e. To do pattern matching on an incoming message, first
	 * parse it, and then construct a match template, filling in the fields that you
	 * want to match. The rules for matching are: A null object matches wild card -
	 * that is a match template of null matches any parsed SIP object. To match with
	 * any subfield, set the match template on a template object of the same type
	 * and invoke the match interface. Regular expressions matching implements the
	 * gov.nist.sip.Match interface that can be done using the Jakarta regexp
	 * package for example. package included herein. This can be used to implement
	 * the Match interface <a href=http://www.apache.org> See the APACHE website for
	 * documents </a>
	 *
	 */
	...
	/**
	 * Clones the given object. If the object is a wrapped type, an array, a
	 * GenericObject or a GenericObjectList, it is cast to the appropriate type and
	 * the clone() method is invoked. Else if the object implements Cloneable,
	 * reflection is used to discover and invoke clone() method. Otherwise, the
	 ...
	 * object. This is useful when you want to genrate a template and override the
	 * fields of an incoming SIPMessage with another SIP message that you have
	 * already generated.
	 *
	 * @param mergeObject is the replacement object. The override obect must be of
	 *                    the same class as this object. Set any fields that you do
	...
	/**
	 * An introspection based predicate matching using a template object. Allows for
	 * partial match of two protocl Objects.
	 * 
	 * @param other the match pattern to test against. The match object has to be of
	 *              the same type (class). Primitive types and non-sip fields that
	 *              are non null are matched for equality. Null in any field matches
	 *              anything. Some book-keeping fields are ignored when making the
	 *              comparison.
	 */
	...
	/**
	 * Generic print formatting function: Does depth-first descent of the structure
	 * and recursively prints all non-private objects pointed to by this object.
	 * <bf> Warning - the following generic string routine will bomb (go into
	 * infinite loop) if there are any circularly linked structures so if you have
	 * these, they had better be private! </bf> We dont have to worry about such
	 * things for our structures (we never use circular linked structures).
	 */
