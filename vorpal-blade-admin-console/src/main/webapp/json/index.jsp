...
// jwm - why is JSON data showing schema?
            ...
            function generateForm() {
				...
				console.log("generateForm... inputString[selectedTab]="+inputString[selectedTab]);
...
// jwm - what does route do?		
            ...
            codeMirrors["data"] = CodeMirror(document.getElementById("data"), {
                value: JSON.stringify(demos[selectedDemo][2], null, 4),
                mode: "javascript",
                lineNumbers: true
            });

            codeMirrors["sample"] = CodeMirror(document.getElementById("sample"), {
                value: JSON.stringify(demos[selectedDemo][3], null, 4),
                mode: "javascript",
                lineNumbers: true
            });
 ...
 */
                ...
                // jwm
                console.log("jquery selectedTab: "+selectedTab);
...
//				console.log("jwm2 - setting value = "+inputString[selectedTab]);

				if(selectedTab=="form" && prevTab=="data"){
					console.log("jquery... #1");
					
					inputString.data = codeMirrors[prevTab].getValue();

					console.log("inputString.data="+inputString.data);

					
					if(inputString.data){
						console.log("redering form...");
						

		                bf.render(container, JSON.parse(inputString.data));
						
		                //bf.render(container, inputString.data);

		                
					}else{
						console.log("no inputString.data to render form...");
					}
										
				}else if(selectedTab=="data" && prevTab=="form" ){
					console.log("jquery... #2");

					if(bf.getData()){
						console.log("bf.getData() is not null...");
					}else{
						console.log("bf.getData() is null!");
					}
					
					inputString.data = JSON.stringify(bf.getData(), null, 4);
					if(inputString.data){
						console.log("setting data code mirrors.")
						codeMirrors[selectedTab].setValue(inputString.data);
					}else{
						console.log("no inputString.data to set data code mirrors.")
					}
