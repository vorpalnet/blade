...
public class Roundtrip extends HttpServlet {
	...
	/**
	 * Defines the initial XML for a new, empty diagram. Note that the diagram node
	 * is an application-level entity which contains the transaction counter (TCN)
	 * and the diagram ID. The mxGraphModel is passed to the mxGraph core classes on
	 * the client-side.
	 */
	protected static String emptyDiagram = "<diagram><mxGraphModel>" + "<root>" + "<Workflow value=\"Diagram\" id=\"0\">" + "<mxCell/>" + "</Workflow>"
			+ "<Layer value=\"Default Layer\" id=\"1\">" + "<mxCell parent=\"0\"/>" + "</Layer>" + "<mxCell vertex=\"1\" parent=\"1\" value=\"Hello, World!\">"
			+ "<mxGeometry x=\"120\" y=\"90\" width=\"80\" height=\"40\" as=\"geometry\"/>" + "</mxCell>" + "</root>" + "</mxGraphModel></diagram>";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (id == null || id.length() == 0) {
			...
			if (Integer.parseInt(storedTcn) > Integer.parseInt(tcn)) {
				...
				response.getWriter().println("Diagram was changed by another user.");
		...
		response.getWriter().println("<result id=\"" + id + "\" tcn=\"" + tcn + "\"/>");
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (id != null) {
			...
			String template = mxUtils.readFile(Roundtrip.class.getResource("/com/mxgraph/examples/web/resources/roundtrip.html").getPath());
			...
			response.getWriter().println("<a href=\"?id=new\">Create diagram</a><br>");

			for (Document doc : diagrams.values()) {
				String diagramId = doc.getDocumentElement().getAttribute("id");
				response.getWriter().println("<a href=\"?id=" + diagramId + "\">Diagram " + diagramId + "</a><br>");
			}
		}
	}
}
