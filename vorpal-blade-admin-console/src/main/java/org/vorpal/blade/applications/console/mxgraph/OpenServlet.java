...
public class OpenServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("OpenServlet...");
...
//				}
				...
				if (!format.equals("xml")) {
					if (xml == null || xml.length() == 0) {
						...
						xml = xml.replaceAll("[\\uFFFD\\u0000]*", "");
						writeScript(writer, "try{window.parent.setCurrentXml(decodeURIComponent('" + encodeString(xml) + "'), decodeURIComponent('"
								+ encodeString(filename) + "'));}catch(e){window.parent.showOpenAlert({message:window.parent.mxResources.get('notAUtf8File')});}");
					}
				} else {
					writer.println(xml);
				}
