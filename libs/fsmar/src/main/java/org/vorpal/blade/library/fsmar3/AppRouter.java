...
import org.vorpal.blade.framework.v3.fsmar.AppRouterConfigurationSample;
...
public class AppRouter implements SipApplicationRouter {
	...
	// Un-versioned config name: the engine reads config/custom/vorpal/fsmar.json
	// (the version lives INSIDE the file). FsmarSettingsManager falls back to the
	// legacy fsmar2.json and auto-upgrades a pre-3 config on load.
	protected static String FSMAR = "fsmar";
	...
	public void init() {
		try {
			settingsManager = new FsmarSettingsManager(FSMAR, AppRouterConfiguration.class,
	...
	/// - `${regionLabel}` — routing region label, when the container supplied one
	///   (JSR-289 region label; FSMAR 2 could route on it via `Region-Label`)
	...
	static void publishPseudoVariables(Context ctx, String method, String requestUri, String directive,
			String regionType, String regionLabel, String previous, String callId) {
		if (ctx == null) {
			return;
		}
		...
		ctx.put("regionLabel", nullSafe(regionLabel));
