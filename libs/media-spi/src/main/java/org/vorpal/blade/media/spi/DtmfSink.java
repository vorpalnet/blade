...
/// to the framework's `MediaCallflow.deliverDtmf`, and the framework routes it to the driver through
/// this hook — exactly the app-aware split [MediaSessionRecovery] uses for failover. A driver whose
/// detector is fed only from the media plane (RFC 4733 / in-band tones) simply doesn't implement this.
///
/// Like the rest of this jar, it is deployed on the classloader that loads the driver, so there is
/// one `DtmfSink` Class across the framework and driver tiers and `factory instanceof DtmfSink` holds.
