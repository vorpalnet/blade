...
/// - `contains` — [AttributeSelector]; `${H} contains 'E'`. FSMAR 2 scanned
///   every instance of a repeating header; the v3 selector reads the first.
///   Flagged with a NOTE.
/// - `uri` — [RegexSelector] extracting the bracketed URI; `${H.uri} matches 'E'`.
/// - `value` — [RegexSelector] keeping the part before `;`; clause on `${H.value}`.
...
/// clean `==` otherwise (digits, IPs, dotted numbers).
///
/// ## Fail closed
///
...
///      org.vorpal.blade.library.fsmar3.Fsmar2Converter fsmar2.json fsmar3.json
/// ```
///
...
public final class Fsmar2Converter {
	...
	/// Full-match pattern keeping a parameterable header's value (before `;`).
	...
	private static String convertComparison(String header, String op, String expr, String where,
		...
		if (expr == null || expr.contains("'")) {
			return null; // not representable in an Expression string literal
		}
		...
		case "Region-Label":
			return null; // no v3 pseudo-variable
		case "Request-URI":
			switch (op) {
			case "matches":
			case "address":
		...
		case "address":
			attributeSelector(header, selectors);
			return "${" + key(header) + "} matches '" + expr + "'";

		case "contains":
			attributeSelector(header, selectors);
			result.warnings.add("NOTE: " + where + " 'contains' on '" + header
					+ "' — fsmar2 scanned every instance of a repeating header; v3 reads the first.");
			return "${" + key(header) + "} contains '" + expr + "'";
		...
		case "uri":
			...
			return "${" + key(header) + ".uri} matches '" + expr + "'";

		case "value":
			regexSelector(header, VALUE_PATTERN, selectors);
			return eq("${" + key(header) + ".value}", expr);

		default:
			// fsmar2 treated an unknown operator as a parameter name.
			if (!op.matches("[a-zA-Z][a-zA-Z0-9]*")) {
				return null; // not a legal regex group name
			}
			regexSelector(header, ".*[;?]" + escapeRegex(op) + "=(?<" + op + ">[^;>\\s]*).*", selectors);
			result.warnings.add("NOTE: " + where + " parameter '" + op + "' of '" + header
					+ "' converted via a parameter-extraction regex; eyeball against real traffic.");
			var = "${" + key(header) + "." + op + "}";
			return eq(var, expr);
		}
	}
	...
	private static String eq(String var, String expr) {
		if (expr.chars().anyMatch(Character::isLetter)) {
			return var + " matches '(?i)" + escapeRegex(expr) + "'";
		}
		return var + " == '" + expr + "'";
	}
