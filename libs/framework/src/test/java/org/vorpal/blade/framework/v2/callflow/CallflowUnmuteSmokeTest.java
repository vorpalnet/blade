...
/// Smoke test for [CallflowUnmute]. Behaviorally identical to
/// [CallflowResumeSmokeTest] (both target `sendrecv`); kept separate for
/// caller-side semantic symmetry with [CallflowMuteSmokeTest].
public final class CallflowUnmuteSmokeTest {
	...
	private static void testNoDirection() {
		String out = SdpDirection.force(baseSdp(""), "sendrecv");
	...
	private static void testMultipartSiprec() throws Exception {
		...
		check("siprec.sendrecv", out.contains("a=sendrecv"));
		check("siprec.no-recvonly", !out.contains("a=recvonly"));
