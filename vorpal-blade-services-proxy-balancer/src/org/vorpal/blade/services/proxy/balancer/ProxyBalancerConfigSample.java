...
public class ProxyBalancerConfigSample extends ProxyBalancerConfig {
	...
	public ProxyBalancerConfigSample() throws ServletParseException {
		...
		proxyTier1.getEndpoints().add(ProxyServlet.getSipFactory().createURI("sip:transferor@blade1;status=501;delay=2"));
		proxyTier1.getEndpoints().add(ProxyServlet.getSipFactory().createURI("sip:transferor@blade2;status=502;delay=4"));
		rule1.getTiers().add(proxyTier1);

		ProxyTier proxyTier2 = new ProxyTier();
		proxyTier2.setMode(Mode.serial);
		proxyTier2.setTimeout(15);
		proxyTier2.getEndpoints().add(ProxyServlet.getSipFactory().createURI("sip:transferor@blade3;status=503;delay=2"));
		proxyTier2.getEndpoints().add(ProxyServlet.getSipFactory().createURI("sip:transferor@blade4;status=504;delay=2"));
